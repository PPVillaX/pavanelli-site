...
export default function GalleryAdmin({ initialPhotos }: Props) {
  ...
  async function handleMove(index: number, direction: 'up' | 'down') {
    ...
    newOrder.sort((a, b) => a.display_order - b.display_order || new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
    setPhotos(newOrder);

    try {
      await Promise.all([
        supabase.from('gallery_photos').update({ display_order: target.display_order }).eq('id', current.id),
        supabase.from('gallery_photos').update({ display_order: current.display_order }).eq('id', target.id),
      ]);
      // Revalidate front-end cache
      await fetch('/api/revalidate', { method: 'POST' });
      router.refresh();
    } catch {
    ...
    <div>
      ...
      <div className="mb-6">
        ...
        <p className="text-xs text-brand-gray mt-2">
          Aceita JPG, PNG, WebP. Convertida automaticamente para WebP.
        </p>
      </div>
      ...
      {photos.length === 0 ? (
        <div className="border-2 border-dashed border-gray-200 rounded-lg py-20 text-center">
          <p className="text-brand-gray text-sm">Nenhuma foto ainda. Adicione a primeira!</p>
        </div>
      ) : (
      ...
      {photos.length > 0 && (
        <p className="text-xs text-brand-gray mt-4">
          {photos.length} foto{photos.length !== 1 ? 's' : ''} · Passe o mouse sobre uma foto para ver as ações
        </p>
      )}
    </div>
  );
